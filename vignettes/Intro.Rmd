...
__SC19092__ is my first R package containing two functions implementing two useful methods I learnt during this semester, including examples. To be concrete, the first function is designed for my undergraduate thesis. It considers the problem of change point detection with FDR control. The second function concerns nonparametric test of independence between two random variables. The solutions to all homework are also attached here for future purpose.
...
Consider a linear model with s changepoints located at $a_1, \dots, a_s$
...
Here is an example containing 4 changepoints. Let $(a_1, a_2,a_3,a_4) = (200,400,600,800)$. 
...
So we see the test reject the null hypothesis. And clearly there are two clusters in the scattor plot, so it is unlikely that the two variables are independent.
